...
@section('page-title', 'Editar evento académico')
@section('page-subtitle', 'Actualizar información registrada del calendario académico')
    ...
    <div class="card border-0 shadow-sm rounded-4">
        <div class="card-body p-4">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h2 class="h5 fw-bold mb-1">Editar evento</h2>
                    <p class="text-muted mb-0">Modifique los datos necesarios del evento académico.</p>
                </div>
            ...
            <form method="POST" action="{{ route('academic-calendar-events.update', $event) }}">
                @csrf
                @method('PUT')
                ...
                <div class="row g-3">
                    ...
                    <div class="col-12 col-md-4">
                        ...
                        <select id="event_type" name="event_type" class="form-select rounded-pill @error('event_type') is-invalid @enderror" required>
                            ...
                            <option value="Académico" @selected(old('event_type', $event->event_type) === 'Académico')>Académico</option>
                            <option value="Evaluación" @selected(old('event_type', $event->event_type) === 'Evaluación')>Evaluación</option>
                            <option value="Reunión" @selected(old('event_type', $event->event_type) === 'Reunión')>Reunión</option>
                            <option value="Feriado" @selected(old('event_type', $event->event_type) === 'Feriado')>Feriado</option>
                            <option value="Actividad especial" @selected(old('event_type', $event->event_type) === 'Actividad especial')>Actividad especial</option>
                        </select>
