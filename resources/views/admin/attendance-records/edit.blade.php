...
@section('page-subtitle', 'Actualizar información registrada del control de asistencia')
    ...
    <div class="card border-0 shadow-sm rounded-4">
        <div class="card-body p-4">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h2 class="h5 fw-bold mb-1">Editar asistencia</h2>
                    <p class="text-muted mb-0">Modifique los datos necesarios del registro.</p>
                </div>
            ...
            <form method="POST" action="{{ route('attendance-records.update', $attendanceRecord) }}">
                @csrf
                @method('PUT')
                ...
                <div class="row g-3">
                    <div class="col-12 col-md-6">
                        ...
                        <select id="student_id" name="student_id" class="form-select rounded-pill @error('student_id') is-invalid @enderror" required>
                            ...
                            @foreach ($students as $student)
                                <option value="{{ $student->id }}" @selected(old('student_id', $attendanceRecord->student_id) == $student->id)>
                    ...
                    <div class="col-12 col-md-6">
                        ...
                        <select id="course_id" name="course_id" class="form-select rounded-pill @error('course_id') is-invalid @enderror" required>
                            ...
                            @foreach ($courses as $course)
                                <option value="{{ $course->id }}" @selected(old('course_id', $attendanceRecord->course_id) == $course->id)>
                    ...
                    <div class="col-12 col-md-6">
                        ...
                        <select id="teacher_id" name="teacher_id" class="form-select rounded-pill @error('teacher_id') is-invalid @enderror">
                            ...
                            @foreach ($teachers as $teacher)
                                <option value="{{ $teacher->id }}" @selected(old('teacher_id', $attendanceRecord->teacher_id) == $teacher->id)>
                    ...
                    <div class="col-12 col-md-6">
                        ...
                        <input
                            ...
                            value="{{ old('attendance_date', $attendanceRecord->attendance_date) }}"
                    ...
                    <div class="col-12">
                        ...
                        <select id="status" name="status" class="form-select rounded-pill @error('status') is-invalid @enderror" required>
                            <option value="presente" @selected(old('status', $attendanceRecord->status) === 'presente')>Presente</option>
                            <option value="ausente" @selected(old('status', $attendanceRecord->status) === 'ausente')>Ausente</option>
                            <option value="tarde" @selected(old('status', $attendanceRecord->status) === 'tarde')>Tarde</option>
                            <option value="justificado" @selected(old('status', $attendanceRecord->status) === 'justificado')>Justificado</option>
                            <option value="anulado" @selected(old('status', $attendanceRecord->status) === 'anulado')>Anulado</option>
                        </select>
                    ...
                    <div class="col-12">
                        ...
                        >{{ old('comments', $attendanceRecord->comments) }}</textarea>
